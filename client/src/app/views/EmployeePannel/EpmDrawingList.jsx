import { Button, Icon, Chip, IconButton, styled,Grid,TableCell, TableHead, TablePagination, TableRow, } from "@mui/material";
...
import { getDrawingListByEmp,} from "app/redux/actions/DrawingAction";
...
const EmpDrawingList = () => {
    ...
    const drawingData = useSelector(state => state?.drawings.empdrawingList);

    useEffect(() => {
        const user = JSON.parse(localStorage.getItem('user'));
        if (user) {
            dispatch(getDrawingListByEmp(user.user.first_name + " " + user.user.last_name));
        }
    }, [dispatch]);
            ...
            <Container>
                ...
                {user === "Employee" ?
                    <Button variant="contained" color="primary" style={{ marginBottom: "10px" }}>
                        <Link color="primary" variant="contained" to="/drawing/new-drawing">
                            Add New Drawing
                        </Link>
                    </Button>
                    : null}
                                                ...
                                                <StylPopUp key={index} sx={{ mb: 2, background: `linear-gradient(87deg,#11cdef 0,#1171ef 100%)!important` }}>
                                                    <Grid container spacing={2}>
                                                        ...
                                                        <Grid item sm={2} style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                                                            <Link color="primary" variant="contained" to="/drawing/edit-drawing" state={subscriber}>
                                                                <IconButton
