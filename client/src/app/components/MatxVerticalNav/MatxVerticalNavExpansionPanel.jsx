...
const MatxVerticalNavExpansionPanel = ({ item, children, mode }) => {
  ...
  const { name, icon, iconText, badge, showTo } = item;
  ...
  return (
    <NavExpandRoot>
      {showTo === user.user_type ? <BaseButton
        className={clsx({
      ...
      >
        <Box display="flex" alignItems="center">
          {showTo === user.user_type ? icon && <Icon className="icon">{icon}</Icon> : null}
          {iconText && <BulletIcon />}
          {showTo === user.user_type ? <ItemText className="sidenavHoverShow">{name}</ItemText> : null}
        </Box>
